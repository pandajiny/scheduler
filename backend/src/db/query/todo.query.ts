...
export function selectTodoQuery(args: {
  userId?: string;
  parentId?: string;
  groupId?: string;
}): string {
...
export function insertTodoQuery(args: { request: AddTodoRequest }): string {
...
export function deleteTodoQuery(args: { todo_id: string }) {
  const { todo_id } = args;
  return `
      DELETE FROM ${DB_TABLES.TODO_TABLE}
      WHERE
      todo_id = "${todo_id}";
    `;
}

export function updateTodoQuery(args: { todo: Todo }) {
  const {
    todo_id,
    content,
    create_datetime,
  ...
  return `
        UPDATE ${DB_TABLES.TODO_TABLE} SET
